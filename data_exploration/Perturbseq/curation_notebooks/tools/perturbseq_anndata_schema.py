...
class ObsSchema(DataFrameModel):
    ...
    perturbed_target_ensg: Series[str] = Field(
        nullable=False, isin=gene_ont.ensembl_gene_id.values
    )
    perturbed_target_symbol: Optional[Series[str]] = Field(
        nullable=True, isin=gene_ont.symbol.values
    )
    perturbed_target_category: Optional[Series[str]] = Field(
    ...
    timepoint: Optional[Series[str]] = Field(nullable=True, regex = r"^P\d+DT\d{1,2}H\d{1,2}M\d{1,2}S$") # /^P\d+DT\d{1,2}H\d{1,2}M\d{1,2}S$/gm
    ...
    tissue_label: Optional[Series[str]] = Field(
    ...
    developmental_stage_id: Optional[Series[str]] = Field(nullable=True, str_contains=":")
