...
def pivot_gene_records(raw_results):
    ...
    for r in raw_results:
        ...
        rows.append(
            {
                # FIX (Aug 18): API renamed this field from "gene_name" to
                # "gene_symbol" — confirmed against the live API response,
                # and against exp_002's corpus (generated Jul 10) which
                # relied on "gene_name" and has correct gene data, proving
                # the field existed under that name at the time.
                "gene": perturbation.get("gene_symbol", "unknown"),
                "score_name": effect.get("score_name", "unknown"),
                "score_value": float(effect.get("score_value", np.nan)) if effect.get("score_value") not in (None, "") else np.nan,
