...
def edit_mavedb_metadata_df_columns(metadata_df: pd.DataFrame = None) -> pd.DataFrame:
    ...
    output_df['experiment_title'] = output_df['experiment_title'] + '; ' + metadata_df['scoreset_title']

    return output_df

def process_mavedb_metadata(mavedb_metadata_json_path: str = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    for d in mdb:
        ...
        if experiment_set_df['primaryPublicationIdentifiers'][0] == [] and experiment_set_df['secondaryPublicationIdentifiers'][0] == []:
            publication_df = pd.DataFrame(columns=['publication_title', 'publication_abstract', 'publication_doi', 'publication_publicationYear', 'publication_url', 'publication_first_author', 'publication_last_author', 'index_urn1'])
        else:
            if experiment_set_df['primaryPublicationIdentifiers'][0] != []:
                publication_df = pd.json_normalize(experiment_set_df['primaryPublicationIdentifiers'][0][0])
            else:
                publication_df = pd.json_normalize(experiment_set_df['secondaryPublicationIdentifiers'][0][0])
            publication_df['first_author'] = ' '.join(publication_df['authors'][0][0]['name'].split(', ')[::-1])
            publication_df['last_author'] = ' '.join(publication_df['authors'][0][-1]['name'].split(', ')[::-1])
            publication_df = publication_df[['title', 'abstract', 'doi', 'publicationYear', 'url', 'first_author', 'last_author']]
            publication_df.columns = ['publication_' + col for col in publication_df.columns]
            publication_df['index_urn1'] = urn1

        experiment_set_df = experiment_set_df[['title', 'shortDescription', 'methodText']]
        experiment_set_df.columns = ['experiment_' + col for col in experiment_set_df.columns]
        experiment_set_df['index_urn1'] = urn1
        experiment_set_df['index_urn2'] = urn2
        experiment_set_df = pd.merge(experiment_set_df, publication_df, how='left', on='index_urn1')
        ...
        for i, exp in enumerate(experiment_set):
            ...
            for score in score_set:
                ...
                if target_genes_df['externalIdentifiers'][0] != []:
                    identifiers_df = pd.json_normalize(target_genes_df['externalIdentifiers'][0])
                    identifiers_df = identifiers_df[['identifier.dbName', 'identifier.identifier']]
                    identifiers_df = identifiers_df.set_index('identifier.dbName').T.reset_index(drop=True)
                    identifiers_df.columns = ['identifier_' + col for col in identifiers_df.columns]
                    identifiers_df['index_urn3'] = urn3_i
                else:
                    identifiers_df = pd.DataFrame(columns=['identifier_Ensembl', 'identifier_RefSeq', 'identifier_UniProt', 'index_urn3'])
                ...
                target_genes_df = target_genes_df[['name', 'category', 'targetSequence.taxonomy.organismName']]
                target_genes_df.columns = ['gene_' + col for col in target_genes_df.columns]
                target_genes_df['index_urn3'] = urn3_i

                target_genes_df = pd.merge(target_genes_df, identifiers_df, how='left', on='index_urn3')
                target_genes_df_full = pd.concat([target_genes_df_full, target_genes_df], axis=0)

            score_set_df = pd.merge(score_set_df, target_genes_df_full, how='left', on='index_urn3')
            score_set_df_full = pd.concat([score_set_df_full, score_set_df], axis=0)
        experiment_set_df = pd.merge(experiment_set_df, score_set_df_full, how='left', on='index_urn2')
        processed_df = pd.concat([processed_df, experiment_set_df], axis=0)

    # filter for human only
    processed_df = processed_df[processed_df['gene_targetSequence.taxonomy.organismName'] == 'Homo sapiens']
    ...
    # reorder columns
    processed_df = processed_df[['index_urn1', 'index_urn2', 'index_urn3'] + [col for col in processed_df.columns if col not in ['index_urn1', 'index_urn2', 'index_urn3']]]
    # order by index_urn3
...
def make_adata_mavedb(
    ...
    X_df = X_df.iloc[:,3::] # first three columns are always hgvs ids, take all columns from 4th onwards as these are the scores

    metadata_subset_dict = curated_metadata_df[curated_metadata_df['dataset_id'] == mavedb_id].to_dict(orient='records')[0]

    OBS_df = pd.DataFrame(
        index=X_df.index,
        data=metadata_subset_dict
    )
...
def process_mavedb(
    ...
    curated_h5ad_path = Path(non_curated_h5ad_dir.as_posix().replace('non_curated', 'curated')) / f"{mavedb_dataset_id}_curated.h5ad"
    if curated_h5ad_path.exists():
        if overwrite:
            print(f"♻️ Curated DepMap data for {mavedb_dataset_id} already exists at {curated_h5ad_path}. Overwriting...")
        else:
            print(f"✅ Curated DepMap data for {mavedb_dataset_id} already exists at {curated_h5ad_path}. Skipping processing.")
            return
