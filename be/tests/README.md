...
These are local PostgreSQL integration tests. They are intentionally not part
of CI and use the development database configured by `pc_secrets`.
...
The external development PostgreSQL connection must be reachable from the
current machine. The suite uses the existing
